...
impl Default for Predictor {
    fn default() -> Self {
        Predictor {
            ...
            pattern_history_table: vec![2_u8.pow(COUNTER_BITS); PATTERN_HISTORY_TABLE_LENGTH],
        }
    }
}
...
impl Predictor {
    fn get_table_index(&self, program_counter: u32) -> usize {
        (program_counter as usize ^ self.global_history_register)
            & (PATTERN_HISTORY_TABLE_LENGTH - 1)
    }
    ...
    pub fn update(&mut self, program_counter: u32, branch_taken: bool) {
        let index = self.get_table_index(program_counter);
        if branch_taken {
            self.pattern_history_table[index] += if self.pattern_history_table[index] < 4 {
                1
            } else {
                0
            };
            self.global_history_register =
                (self.global_history_register << 1) | 1 & (PATTERN_HISTORY_TABLE_LENGTH - 1);
        } else {
            self.pattern_history_table[index] -= if self.pattern_history_table[index] > 0 {
                1
            } else {
                0
            };
            self.global_history_register =
                (self.global_history_register << 1) & (PATTERN_HISTORY_TABLE_LENGTH - 1);
        }
    }
}
